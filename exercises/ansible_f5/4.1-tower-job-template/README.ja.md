...
NOTE: パスワードは、学生ラボ情報が含まれているWebページのワークベンチ情報セクションにあります。パスワードがわからない場合は、インストラクターに相談してください。
...
認証情報タイプから `Network` を選択し、次に `BIGIP` を選択します。
...
- username: admin
- password: インストラクターから提供されます。デフォルトは ansible です。
