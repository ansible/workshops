...
これを実現するのが Ansible の Role です。Role という形で Playbook をパーツとして分解し、構造化されたディレクトリに納めるのです。  
詳しくはこちらの [ベストプラクティス](http://docs.ansible.com/ansible/playbooks_best_practices.html) をご確認ください。  

## ステップ 1.7.1 - Ansible Roles 構造を理解する

Role は基本的に、ディレクティブを自動化したものであり、実際には参照ファイルの検索パス処理に対するいくつかの機能を超えた追加の魔法的な手段は含まれていません。

Role は定義されたディレクトリ構造に従い、Role は最上位ディレクトリ名で定義されます。いくつかのサブディレクトリの中には `main.yml` という名前の YAML ファイルが含まれています。 `files` と `templates` のサブディレクトリには YAML ファイルによって参照されるオブジェクトを入れておくことができます。  

一例を下記します。Role の名前は "apache" です。  
...
`main.yml` ファイルは、対応するディレクトリに応じたコンテンツが含まれています。例えば、  `vars/main.yml` では変数が定義され、 `handlers/main.yaml` では、ハンドラが記述される等です。一見 Playbook と似ていますが、これらの `main.yml` ファイルの中には特定のコンテンツのみが含まれるのみで、その他の例えば、 host の `become` やその他のキーワードなど通常のプレイブックに記載される情報は含まれません。
...
Playbook でRole を呼び出すのは以下の通り簡単です。  
...
タスク、ハンドラ、変数など各々の Role がこの順番で Playbook に組み込まれます。 Role で定義されたディレクトリに、コピー、スクリプト、テンプレート、タスクを入れることで、絶対パスや相対パスを意識することなくそれぞれの Role にアクセスすることができます。    

## ステップ 1.7.2 - 基本的なロールディレクトリ構造を作成する    

Ansible は、プロジェクトディレクトリ内の `roles` サブディレクトリの中から該当する Role を探します。これは　Ansible　の設定ファイルで上書きすることも可能です。それぞれの Role は独自のディレクトリ構造を持っています。新規ロールのディレクトリを作成するために `ansible-galaxy` を利用することも可能です。  
...
早速ロールを作成してみましょう。Apache をインストールして仮想ホストとして機能するように設定する Role のディレクトリを構築します。このコマンドは、 `~/ansible-files` ディレクトリで実行してください。  
...
`node2` に対し、 Role をテストする準備が整いました。しかし、 Role はノードに直接割り当てることができないため、まずロールとホストを紐づけるプレイブックを作成します。 `~/ansible-files` ディレクトリの直下に `test_apache_role.yml` ファイルを以下の内容で作成します。
...
`pre_tasks` と `post_tasks` というキーワードに注意してください。通常、 Role のタスクはプレイブックのタスクの前に実行されます。この順番を制御するには、 `pre_tasks` が必要となります。逆に `post_tasks` は、すべてのロールが完了した後に実行されます。ここでは、実際の Role が実行されたときにどういう順番で実行されたかを確認するため、これら二つのタスクをあえて入れています。  
