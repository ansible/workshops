...
Roles は定義されたディレクトリ構造に従い、Role は最上位ディレクトリ名で定義されます。いくつかのサブディレクトリの中には `main.yml` という名前の YAML ファイルが含まれています。 `files` と `templates` のサブディレクトリには YAML ファイルによって参照されるオブジェクトを入れておくことができます。  
