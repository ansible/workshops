...
Next, on the left side menu, click on **SECURITY POLICIES**, note that there is currently only one rule installed: to drop all traffic. 
This is an example of Check Point management interface, we will come back to it later but first, let's go back to the command line to start writing Ansible playbooks to interact with Check Point.
