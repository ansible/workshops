...
Next, we need to add the tasks. Tasks are the the components which make the actual changes on the target machines. Since we are using a role, we can simply use a single step, `include_role` include it.  In order to make the role suitable for our use case, we add the following task-specific variables:
...
And most importantly, we want to be able see what was actually found. The `ids_rule_facts` stores the data it collects as Ansible facts. Ansible facts are information individual to each host which can be used in further tasks. Therefore, we add another task to output these facts.
