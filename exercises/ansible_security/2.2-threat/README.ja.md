...
ブラウザを開き、Tower インスタンスへのリンクを入力します。Student ID と与えられたパスワードでログインします。左側にダッシュボードとナビゲーションバーが表示されます。

![Tower dashboard](images/tower_dashboard.png)

左側の **Templates** をクリックします。設定済みのすべてのジョブテンプレートの一覧が表示されます。ジョブテンプレートは、Ansible ジョブを実行するための定義とパラメータのセットです。インベントリ、認証情報、Playbook、制限、権限など、自動化を実行するために必要なものを定義します。このリストの中で **Blacklist attacker** というエントリを見つけ、その右にあるロケットマークをクリックします:
