...
data "amazon-ami" "automation_hub" {
  filters = {
...
source "amazon-ebs" "automation_hub_source" {
  ami_name             = "automation_hub {{timestamp}}"
  instance_type        = "m4.xlarge"
  region               = "us-east-1"
  associate_public_ip_address = "true"
  source_ami           = data.amazon-ami.automation_hub.id
...
build {
    sources = ["sources.amazon-ebs.automation_hub_source"]

    provisioner "ansible" {
      command = "ansible-playbook"
      playbook_file = "pre_build_hub.yml"
      user = "ec2-user"
