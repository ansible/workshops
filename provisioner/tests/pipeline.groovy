...
workshop_type: rhel
ec2_name_prefix: tqe-rhel-tower${DOTLESS_TOWER_VERSION}-${env.BRANCH_NAME}-${env.BUILD_ID}-${SHORTENED_ANSIBLE_VERSION}
EOF
"""
...
ec2_name_prefix: tqe-networking-tower${DOTLESS_TOWER_VERSION}-${env.BRANCH_NAME}-${env.BUILD_ID}-${SHORTENED_ANSIBLE_VERSION}
EOF
"""
...
ec2_name_prefix: tqe-f5-tower${DOTLESS_TOWER_VERSION}-${env.BRANCH_NAME}-${env.BUILD_ID}-${SHORTENED_ANSIBLE_VERSION}
EOF
"""
...
ec2_name_prefix: tqe-security-tower${DOTLESS_TOWER_VERSION}-${env.BRANCH_NAME}-${env.BUILD_ID}-${SHORTENED_ANSIBLE_VERSION}
EOF
"""
            ...
            parallel {
                ...
                stage('F5') {
                    steps {
                        ...
                        script {
                            stage('F5-exercises') {
                                sh "cat provisioner/tqe-f5-tower${DOTLESS_TOWER_VERSION}-${env.BRANCH_NAME}-${env.BUILD_ID}-${SHORTENED_ANSIBLE_VERSION}/student1-instances.txt | grep -A 1 control | tail -n 1 | cut -d' ' -f 2 | cut -d'=' -f2 | tee control_host"
